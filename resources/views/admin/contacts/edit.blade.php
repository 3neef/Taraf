...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.contacts.update", [$contact->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="additional_phone">{{ trans('cruds.contact.fields.additional_phone') }}</label>
                <input class="form-control {{ $errors->has('additional_phone') ? 'is-invalid' : '' }}" type="text" name="additional_phone" id="additional_phone" value="{{ old('additional_phone', $contact->additional_phone) }}">
                @if($errors->has('additional_phone'))
                    <div class="invalid-feedback">
                        {{ $errors->first('additional_phone') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.contact.fields.additional_phone_helper') }}</span>
            </div>
            <div class="form-group">
