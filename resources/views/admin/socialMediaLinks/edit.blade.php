    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.social-media-links.update", [$socialMediaLink->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $socialMediaLink->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('url') ? 'is-invalid' : '' }}" name="url" id="url" required>{{ old('url', $socialMediaLink->url) }}</textarea>
                @if($errors->has('url'))
                    <div class="invalid-feedback">
                        {{ $errors->first('url') }}
                    </div>
