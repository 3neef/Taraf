...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.clients.update", [$client->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required">{{ trans('cruds.client.fields.is_partner') }}</label>
                @foreach(App\Models\Client::IS_PARTNER_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('is_partner') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="is_partner_{{ $key }}" name="is_partner" value="{{ $key }}" {{ old('is_partner', $client->is_partner) === (string) $key ? 'checked' : '' }} required>
                        <label class="form-check-label" for="is_partner_{{ $key }}">{{ $label }}</label>
                    </div>
                @endforeach
                @if($errors->has('is_partner'))
                    <div class="invalid-feedback">
                        {{ $errors->first('is_partner') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.client.fields.is_partner_helper') }}</span>
            </div>
            <div class="form-group">
